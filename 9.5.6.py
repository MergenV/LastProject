...
def audio_to_text():
    with open(f'{name_audio_file}.mp3', 'rb') as files:
...
def main():
    ...
    with uc.Chrome(main_version=109, chrome_options=options, seleniumwire_options=prx_1) as browser:
        articles = []
        for x in range(1,7):
            url = f'https://captcha-parsinger.ru/v2?page={x}'
            browser.get(url)
            ...
            if browser.find_elements(By.TAG_NAME, "iframe"):
                ...
                articles.extend([int(el.text.split()[1]) for el in
